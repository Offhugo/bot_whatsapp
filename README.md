...
- **DTOs** define and validate data entering the application.
- The **Service Layer** coordinates application behavior.
- **Use Cases** represent application-specific operations.
- The **AI Service** isolates communication with OpenAI.
...
The application identifies the user associated with the incoming message and processes it according to the application's defined behavior.
...
The AI layer interprets the user's natural-language message according to the application's defined prompt and response structure.
...
The system is structured so individual components can be tested in isolation when appropriate, while integrated flows can be tested when interactions between components need to be verified.
...
The objective is to verify not only individual functions but also the behavior of the application when its components interact.
    ...
    Tests         Tests    Tests
      |            |      |
      v            v      v
    Isolated    Component  HTTP /
    behavior    interaction application
                           behavior
...
**Mocks** are used when an external dependency should not participate in the test.
...
- **Pytest**
- **JUnit-style unit testing concepts**
- **Integration Testing**
- **API Testing**
- **Mocks**
- **Fixtures**
...
[LinkedIn](https://linkedin.com/in/offhugo) · [GitHub](https://github.com/Offhugo)
